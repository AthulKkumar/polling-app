...
let AuthController = class AuthController {
    ...
    async register(registerUserDto) {
        const { token, userId, role } = await this.authService.registerUser(registerUserDto);
        return { accessToken: token, userId, role };
    }
    async login(registerUserDto) {
        const { token, userId, role } = await this.authService.loginUser(registerUserDto.email, registerUserDto.password);
        return { accessToken: token, userId, role };
    }
    getProfile(req) {
        return req.user;
